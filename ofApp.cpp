...
void ofApp::setup() {
	...
	for (int i = 0; i < 10; i++) {
		LittleBlocks* block = new LittleBlocks;
		block->y = ofRandom(-65, -100);
		block->x = ofRandom(0, ofGetWidth());
		blocks.push_back(new LittleBlocks);
	}
...
void ofApp::update() {
	...
	myfile << "Gameover you have BLOCKED" << endl << "You have reached Level : "<< 
		level << endl << "Final Score: " << astro.allscore << endl<< "THANK YOU FOR PLAYING" << endl;
	myfile.close();
}
